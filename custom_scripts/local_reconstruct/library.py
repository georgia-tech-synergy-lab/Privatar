...
def test_img_dct_transform(x, bs, ch, h, w):
    ...
    # Write Image to outside
    for i in range(dct_block_reorder.shape[2]):
        # print(f"temp_img_block:{dct_block_reorder[:,:,i,:,:].shape}")
        temp_img_block = dct_block_reorder[:,:,i,:,:].squeeze(0)
        # print(f"temp_img_block:{temp_img_block.shape}")
        # temp_img_block = temp_img_block.permute(1,0,2,3).squeeze(0)
        # print(temp_img_block.shape)
        temp_img_block += 128
        temp_img_block = dct.to_rgb(temp_img_block)  # comvert RGB to YCBCR
        temp_img_block = (temp_img_block / 255 * 2) - 1
        # print(torch.sum(temp_img_block))
        transforms.functional.to_pil_image(temp_img_block).save(f'post_bdct_4x4_{i}.png')
