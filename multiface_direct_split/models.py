...
class DeepAppearanceVAEDirectSplit(nn.Module):
    def __init__(
        ...
        z_dim = n_latent if mode == "vae" else n_latent * 2
        self.mode = mode
        self.enc = DeepApperanceEncoderDirectSplit(
            tex_size, mesh_inp_size, n_latent=z_dim, res=res
        )
        self.dec = DeepAppearanceDecoderDirectSplit(
            tex_size, mesh_inp_size, z_dim=z_dim, res=res, non=non, bilinear=bilinear
        )
        self.cc = ColorCorrection(n_cams)
    ...
    def forward(self, avgtex, mesh, view, cams=None):
        b, n, _ = mesh.shape
        mesh = mesh.view((b, -1))
        mean_mesh, logstd_mesh, mean_texture, logstd_texture = self.enc(avgtex, mesh)
        mean_mesh = mean_mesh * 0.1
        logstd_mesh = logstd_mesh * 0.01
        if self.mode == "vae":
            kl_mesh = 0.5 * torch.mean(torch.exp(2 * logstd_mesh) + mean_mesh**2 - 1.0 - 2 * logstd_mesh)
            std_mesh = torch.exp(logstd_mesh)
            eps_mesh = torch.randn_like(mean_mesh)
            z_mesh = mean_mesh + std_mesh * eps_mesh
        else:
            z_mesh = torch.cat((mean_mesh, logstd_mesh), -1)
            kl_mesh = torch.tensor(0).to(z_mesh.device)

        if self.mode == "vae":
            kl_texture = 0.5 * torch.mean(torch.exp(2 * logstd_texture) + mean_texture**2 - 1.0 - 2 * logstd_texture)
            std_texture = torch.exp(logstd_texture)
            eps_texture = torch.randn_like(mean_texture)
            z_texture = mean_texture + std_texture * eps_texture
        else:
...
class DeepAppearanceDecoderDirectSplit(nn.Module):
    def __init__(
        self, tex_size, mesh_size, z_dim=128, res=False, non=False, bilinear=False
    ):
        super(DeepAppearanceDecoderDirectSplit, self).__init__()
    ...
    def forward(self, z_mesh, z_texture, v):
        view_code = self.relu(self.view_fc(v))
        z_code_texture = self.relu(self.z_fc(z_texture))
        feat_texture = torch.cat((view_code, z_code_texture), 1)
        texture_code = self.relu(self.texture_fc(feat_texture))
        texture = self.texture_decoder(texture_code)
        
        mesh = self.mesh_fc(z_mesh)
...
class DeepApperanceEncoderDirectSplit(nn.Module):
    def __init__(self, inp_size=1024, mesh_inp_size=21918, n_latent=128, res=False):
        super(DeepApperanceEncoderDirectSplit, self).__init__()
        ...
        self.fc_mesh = LinearWN(256, n_latent * 2)
        self.fc_texture = LinearWN(256, n_latent * 2)
        self.relu = nn.LeakyReLU(0.2, inplace=True)

        self.apply(lambda x: glorot(x, 0.2))
        glorot(self.fc_mesh, 1.0)
        glorot(self.fc_texture, 1.0)
    ...
    def forward(self, tex, mesh):
        ...
        latent_texture = self.fc_texture(tex_feat)
        latent_mesh = self.fc_mesh(mesh_feat)
        return latent_mesh[:, : self.n_latent], latent_mesh[:, self.n_latent :], latent_texture[:, : self.n_latent], latent_texture[:, self.n_latent :]
