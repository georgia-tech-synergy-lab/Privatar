...
def main(args, camera_config):
    ...
    attacker_model.load_state_dict(torch.load(args.nn_attacker_model_path, map_location=device))
    ...
    for epoch in range(args.epoch):  # loop over the dataset multiple times
        for i, data in tqdm(enumerate(attack_loader)):
            ...
            if outputs.argmax() == reference_output:
                print(f"Expression: {glb_expression}, and its reference output: {reference_output}, Camera: {glb_camera_id}, Frame: {glb_frame_name}")
                correct_guess += 1
            total_guess += 1
        if wandb_enable:
    ...
    end_time = time.time()
    print("Attacker Testing takes %f seconds" % (end_time - begin_time))
    print(
        "successful rate %f"
        % (correct_guess/total_guess)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nn_attacker_model_path",
        type=str,
        default=None,
        help="NN Attacker model path",
    )
