...
def main(args, camera_config):
    ...
    if not accumulate_channel:
        for i, data in tqdm(enumerate(attack_loader)):
            ...
            projected_offloaded_freq_comp = torch.zeros((bs, model.offloaded_channel_ratio*ch, block_num, block_num))
            overall_channel = model.offloaded_channel_ratio - 1
            ...
            for index, freq_pair in enumerate(model.sorted_index_array):
                if model.normalize_list[freq_pair[0]] + model.normalize_list[freq_pair[1]] < 2:
                    # merge two into one.
                    projected_offloaded_freq_comp[0,overall_channel*ch:(overall_channel+1)*ch,:,:] = dct_block_reorder[0,ch*freq_pair[0]:ch*(freq_pair[0]+1),:,:] + dct_block_reorder[0,ch*freq_pair[1]:ch*(freq_pair[1]+1),:,:]
                    overall_channel = overall_channel - 1
                    if overall_channel < 0:
                      break
                else:
                    projected_offloaded_freq_comp[0,overall_channel*ch:(overall_channel+1)*ch,:,:] = dct_block_reorder[0,ch*freq_pair[0]:ch*(freq_pair[0]+1),:,:]
                    overall_channel = overall_channel - 1
                    projected_offloaded_freq_comp[0,overall_channel*ch:(overall_channel+1)*ch,:,:] = dct_block_reorder[0,ch*freq_pair[1]:ch*(freq_pair[1]+1),:,:]
                    overall_channel = overall_channel - 1
                    if overall_channel < 0:
                      break
            expressions_freq_comps.append(projected_offloaded_freq_comp)
    # version 2: only reconstruct higher frequency components
        ...
        for i, data in tqdm(enumerate(attack_loader)):
            ...
            block_num = h // model.block_size
            dct_block_reorder = model.dct_transform(avg_tex, bs, ch, h, w)
            dct_block_reorder = dct_block_reorder[:, model.total_frequency_component-len(model.offloaded_freq_list):model.total_frequency_component, :, :, :].reshape(bs, ch*len(model.offloaded_freq_list), block_num, block_num)
    ...
    # version 3: accumulate all frequency components into an aggregated synthesized component
        ...
        for i, data in tqdm(enumerate(attack_loader)):
            ...
            block_num = h // model.block_size
            dct_block_reorder = model.dct_transform(avg_tex, bs, ch, h, w)
            dct_block_reorder = dct_block_reorder[:, model.offloaded_freq_list, :, :, :].reshape(bs, ch*len(model.offloaded_freq_list), block_num, block_num)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--loss_weight_mask",
        type=str,
        default="/work/experiment_scripts/loss_weight_mask.png",
    ...
    parser.add_argument(
        "--num_freq_comp_offloaded", type=int, default=2, help="number of offloaded component 2,4,6,8,10,12,14"
    )
    parser.add_argument(
        "--save_latent_code",
        action='store_true',
