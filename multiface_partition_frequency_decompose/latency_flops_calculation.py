...
for num_freq_comp_offloaded in num_freq_comp_offloaded_list:
    ...
    model = DeepAppearanceVAE_Partition(
        1024, 21918, n_latent=256, n_cams=38, num_freq_comp_offloaded=num_freq_comp_offloaded, result_path="/tmp/tmp", save_latent_code=False
    ).to(device)
    ...
    class TracedDecoder(torch.nn.Module):
        ...
        def forward(self, v, z_local, texture_offload):            
    ...
    sample_texture_offload = texture_offload  # Use the actual output from above

    # Trace the model
    with torch.no_grad():
        traced_model = torch.jit.trace(traced_decoder, (sample_v, sample_z_local, sample_texture_offload))

    # Calculate FLOPs for the traced model
    print(f"\n=== FLOPs Analysis for num_freq_comp_offloaded={num_freq_comp_offloaded} ===")
    ...
    input_shapes = [sample_v.shape, sample_z_local.shape, sample_texture_offload.shape]
    estimated_flops = calculate_flops_manual(traced_decoder, input_shapes)
    
    # If thop is available, try it but don't rely on it
    if THOP_AVAILABLE:
        try:
            flops, params = profile(traced_decoder, inputs=(sample_v, sample_z_local, sample_texture_offload), verbose=False)
            if flops > 0:
    ...
    try:
        ...
        macs = profile_macs(traced_decoder, (sample_v, sample_z_local, sample_texture_offload))
        if macs > 0:
    ...
    flops_data[num_freq_comp_offloaded] = {
    ...
    if TORCHSUMMARY_AVAILABLE:
        print(f"\n=== Model Summary for num_freq_comp_offloaded={num_freq_comp_offloaded} ===")
        try:
            if TORCHINFO_AVAILABLE:
                # Use torchinfo for better multi-input support
                summary_str = summary(
                    traced_decoder, 
                    input_data=[sample_v, sample_z_local, sample_texture_offload],
                    verbose=0,
                ...
                print(f"  texture_offload: {sample_texture_offload.shape}")
        except Exception as e:
    ...
    with torch.no_grad():
        for _ in range(10):
            _ = traced_model(sample_v, sample_z_local, sample_texture_offload)
    ...
    if performance_test:
        ...
        for _ in tqdm(range(total_inference)):
            ...
            with torch.no_grad():
                _ = traced_model(v, z_local, texture_offload)
