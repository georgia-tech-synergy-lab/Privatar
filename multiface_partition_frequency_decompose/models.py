...
class DeepAppearanceVAE_Partition(nn.Module):
    def __init__(
        self,
        tex_size=1024,
        mesh_inp_size=21918,
        ...
        num_freq_comp_offloaded=4, # must be multiple of 2
        result_path=False,
        save_latent_code=False,
        gaussian_noise_covariance_path=None
    ):
        super(DeepAppearanceVAE_Partition, self).__init__()
        n_blocks = 4
        ...
        self.mode = mode
        self.offloaded_channel_ratio, self.local_channel_ratio, self.local_freq_list, self.offloaded_freq_list, self.form_group, self.normalize_list, self.sorted_index_array = self.generate_freq_group_index(l2_diff_list, num_freq_comp_offloaded)
        ...
        self.enc_offloaded = DeepApperanceEncoderLayerRed(
            tex_size, mesh_inp_size, n_latent=z_dim, res=res, channel_ratio=len(self.offloaded_freq_list)
        )

        self.dec = DeepAppearanceDecoderLayerRedChnlExpand(
            tex_size, mesh_inp_size, z_dim=z_dim, res=res, non=non, bilinear=bilinear, local_channel_ratio=self.local_channel_ratio, offloaded_channel_ratio=self.offloaded_channel_ratio
        )
    ...
    def generate_freq_group_index(self, l2_diff_list, num_freq_comp_offloaded=4):
        ...
        offloaded_channel_ratio = 0
        offloaded_freq_list = []
        for index, freq_pair  in enumerate(sorted_index_array):
            offloaded_freq_list.append(freq_pair[0])
            offloaded_freq_list.append(freq_pair[1])
            offloaded_channel_ratio += index_freq_div[index]
            if len(offloaded_freq_list) == num_freq_comp_offloaded:
                break
        local_freq_list = [i for i in range(self.total_frequency_component) if i not in offloaded_freq_list ]
        local_channel_ratio = np.sum(np.round(form_group)) - offloaded_channel_ratio
        return int(offloaded_channel_ratio), int(local_channel_ratio), local_freq_list, offloaded_freq_list, form_group, normalize_list, sorted_index_array
    ...
    def forward(self, avgtex, mesh, view, cams=None):
        ...
        dct_block_reorder_offload = dct_block_reorder[:,self.offloaded_freq_list,:,:,:]
        ...
        dct_block_reorder_offload = dct_block_reorder_offload.reshape(bs, ch*len(self.offloaded_freq_list), block_num, block_num) # into (bs, ch, block_num, block_num, 16)
        # print(f"dct_block_reorder_local.shape={dct_block_reorder_local.shape}")
        # print(f"dct_block_reorder_offload.shape={dct_block_reorder_offload.shape}")
        ...
        mean_offload, logstd_offload = self.enc_offloaded(dct_block_reorder_offload, mesh)
        mean_offload = mean_offload * 0.1
        logstd_offload = logstd_offload * 0.01
        if self.mode == "vae":
            kl_offload = 0.5 * torch.mean(torch.exp(2 * logstd_offload) + mean_offload**2 - 1.0 - 2 * logstd_offload)
            std_offload = torch.exp(logstd_offload)
            eps_offload = torch.randn_like(mean_offload)
            z_offload = mean_offload + std_offload * eps_offload
            if self.save_latent_code:
                torch.save(z_offload, f"{self.latent_code_path}/z_offload_{self.iter}.pth")
                self.iter = self.iter + 1
        else:
            z_offload = torch.cat((mean_offload, logstd_offload), -1)
            kl_offload = torch.tensor(0).to(z.device)

        #######################
        ## Add noise to whole latent code
        if self.apply_gaussian_noise:
            samples = torch.from_numpy(np.random.multivariate_normal(self.mean, self.gaussian_noise_covariance, z_offload.shape[0]))
            samples = samples.to(z_offload.device)
            samples = samples.to(z_offload.dtype)
            z_offload = z_offload + samples
        #######################

        kl_merge = len(self.local_freq_list)/self.total_frequency_component*kl + len(self.offloaded_freq_list)/self.total_frequency_component*kl_offload

        pred_tex_freq_components, pred_mesh = self.dec(z, z_offload, view)
        pred_tex = self.dct_inverse_transform(pred_tex_freq_components, bs, ch, h, w)
    ...
    def attack_forward(self, avgtex, mesh, view, cams=None):
        ...
        texture_offload = self.dec.attack_forward(z_offload, view)
...
class DeepAppearanceDecoderLayerRedChnlExpand(nn.Module):
    def __init__(
        self, tex_size, mesh_size, z_dim=128, res=False, non=False, bilinear=False, local_channel_ratio=12, offloaded_channel_ratio=4
    ):
        super(DeepAppearanceDecoderLayerRedChnlExpand, self).__init__()
        nhidden = z_dim * 4 * 4 if tex_size == 1024 else z_dim * 2 * 2
        self.texture_decoder_local = TextureDecoderLayerRedChnlExpand(
            tex_size, z_dim, res=res, non=non, bilinear=bilinear, channel_ratio=local_channel_ratio
        )
        self.texture_decoder_offload = TextureDecoderLayerRedChnlExpand(
            tex_size, z_dim, res=res, non=non, bilinear=bilinear, channel_ratio=offloaded_channel_ratio
        )
        self.z_fc_offload = LinearWN(z_dim, 256)
        self.texture_fc_offload = LinearWN(256 + 8, nhidden)
        ...
        glorot(self.texture_decoder_offload.upsample[-1].conv2, 1.0)

    def forward(self, z_local, z_offload, v):
        ...
        z_code_offload = self.relu(self.z_fc_offload(z_offload))
        feat_offload = torch.cat((view_code, z_code_offload), 1)
        texture_code_offload = self.relu(self.texture_fc_offload(feat_offload))
        texture_offload = self.texture_decoder_offload(texture_code_offload)

        texture_merge = torch.cat((texture_local, texture_offload), dim=1)

        return texture_merge, mesh

    def attack_forward(self, z_offload, v):
        view_code = self.relu(self.view_fc(v))
        z_code_offload = self.relu(self.z_fc_offload(z_offload))
        feat_offload = torch.cat((view_code, z_code_offload), 1)
        texture_code_offload = self.relu(self.texture_fc_offload(feat_offload))
...
class DeepAppearanceDecoderLayerRed(nn.Module):
    def __init__(
        ...
        self.texture_decoder_offload = TextureDecoderLayerRed(
            tex_size, z_dim, res=res, non=non, bilinear=bilinear, channel_ratio=offloaded_channel_ratio
        )
        self.z_fc_offload = LinearWN(z_dim, 256)
        self.texture_fc_offload = LinearWN(256 + 8, nhidden)
...
class TextureDecoderLayerRedChnlExpand(nn.Module):
    def __init__(self, tex_size, z_dim, res=False, non=False, bilinear=False, channel_ratio=2):
        super(TextureDecoderLayerRedChnlExpand, self).__init__()
        ...
        self.upsample = nn.Sequential(
            ...
            ConvUpsample(
                int(64/16*channel_ratio), int(64/16*channel_ratio), int(48/16*channel_ratio), base * (2**2), res=res, use_bilinear=bilinear, non=non
            ),
            ConvUpsample(
                int(48/16*channel_ratio), int(48/16*channel_ratio), int(48/16*channel_ratio), base * (2**4), res=res, use_bilinear=bilinear, non=non
            ),
        )
