...
def main(args, camera_config, test_segment):
    ...
    def run_net(data):
        ...
        if args.use_lpips and lpips_model is not None and pred_screen is not None:
            # pred_screen is (batch, width, height, channels), convert to (batch, channels, height, width)
            pred_screen_lpips = pred_screen.permute(0, 3, 2, 1)  # (B, C, H, W)
            # photo_short could be in (B, C, H, W) or (B, H, W, C) format - check and convert
            photo_lpips = photo_short
            if photo_lpips.shape[1] != 3:
                # Photo is in (B, H, W, C) format, convert to (B, C, H, W)
                photo_lpips = photo_lpips.permute(0, 3, 1, 2)  # (B, C, H, W)
            # Ensure both have same spatial dimensions
            if pred_screen_lpips.shape[2:] != photo_lpips.shape[2:]:
                # Resize pred_screen to match photo dimensions
                pred_screen_lpips = F.interpolate(pred_screen_lpips, size=photo_lpips.shape[2:], mode='bilinear', align_corners=False)
            # LPIPS expects values in range [-1, 1], current values are in [0, 1]
    ...
    total, vert, tex_val, screen, kl, lpips_val = [], [], [], [], [], []
    ...
    for i, data in tqdm(enumerate(test_loader)):
        ...
        if glb_camera_id != args.selected_camera:
            continue
        glb_frame_name = str(_first_item(data.get("frame")))
        print(f"Expression: {glb_expression}, Camera: {glb_camera_id}, Frame: {glb_frame_name}")
        losses, output = run_net(data)
        total.append(losses["total_loss"].item())
        vert.append(losses["vert_loss"].item())
        tex_val.append(losses["denorm_tex_loss"].item()) # denormalized
        ...
        if args.save_img:
            save_img(data, output, f"{glb_expression}_{glb_camera_id}_{glb_frame_name}")
            
        if i >= args.val_num:
          break

    total_loss = np.array(total).mean()
    tex_loss = np.array(tex_val).mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--selected_camera", type=str, default=None, help="Selected camera")
    ...
    camera_config_path = f"/work/experiment_scripts/camera_configs/camera-split-config_{subject_id}.json"
