...
def evaluate_single_image(args, camera_config, test_segment, image_path):
    ...
    if target_data is None:
        ...
        if os.path.exists(image_path):
            ...
            if img is not None:
                ...
                # Save the input image as input_expression.png
                if extracted_info:
                    ...
                    pil_img.save(os.path.join(args.result_path, f"input_{sentnum}.png"))
                    print(f"Saved input image as: input_{sentnum}.png")
            else:
    ...
    def save_img_single(data, output, tag="single_eval"):
        ...
        if len(save_pred_tex_image.shape) == 4:
            for _batch_id in range(save_pred_tex_image.shape[0]):
                ...
                Image.fromarray(save_pred_tex_image[_batch_id]).save(os.path.join(args.result_path, f"pred_texture_{tag}_{_batch_id}.png"))
        ...
        if not using_template and "tex" in data:
            ...
            if len(save_gt_tex_image.shape) == 4:
                for _batch_id in range(save_gt_tex_image.shape[0]):
                    ...
                    Image.fromarray(save_gt_tex_image[_batch_id]).save(os.path.join(args.result_path, f"gt_texture_{tag}_{_batch_id}.png"))
        else:
            print(f"Note: Skipping ground truth texture save since using template data (not exact match from dataset)")

        print(f"Saved images:")
        print(f"  - input_{tag}_0.png: Original input image")
    ...
    with torch.no_grad():
        ...
        if args.save_img:
            os.makedirs(args.result_path, exist_ok=True)
            save_img_single(data, output, "single_eval")
