...
if noisy_attack:
    for num_freq_comp_outsourced in num_freq_comp_outsourced_list:
        ...
        for v in mi_list:
            ...
            print(f"num_freq_comp_outsourced={num_freq_comp_outsourced}  mi={v}")
            os.system(f'python test_empirical_attack_run.py --data_dir {data_dir} --krt_dir {data_dir}/KRT --framelist_test {framelist_test}  --result_path {result_path} --test_segment "./test_segment.json" --lambda_screen 1 --model_path {best_model_path}  --arch base  --project_name {project_name} --author_name {wandb_author_name} --val_batch_size {val_batch_size}  --num_freq_comp_outsourced {num_freq_comp_outsourced} --gaussian_noise_covariance_path {gaussian_noise_covariance_path} --camera_configs_path {camera_configs_path}')
else:
    for num_freq_comp_outsourced in num_freq_comp_outsourced_list:
        ...
        if using_pac_noise:
            gaussian_noise_covariance_path = f"{path_to_privatar}/experiment_scripts/pac_analysis/noise_covariance/pac_noise_outsource_ibdct_decoder_{num_freq_comp_outsourced}.npy"
        else: 
            gaussian_noise_covariance_path = f"{path_to_privatar}/experiment_scripts/dp_analysis/generated_dp_noise/dp_noise_ibdct_decoder_{num_freq_comp_outsourced}.npy"
        ...
        print(f'python test_empirical_attack_run.py --data_dir {data_dir} --krt_dir {data_dir}/KRT --framelist_test {framelist_test}  --result_path {result_path} --test_segment "./test_segment.json" --lambda_screen 1 --model_path {best_model_path}  --arch base  --project_name {project_name} --author_name {wandb_author_name} --val_batch_size {val_batch_size}  --num_freq_comp_outsourced {num_freq_comp_outsourced} --camera_configs_path {camera_configs_path}')
        os.system(f'python test_empirical_attack_run.py --data_dir {data_dir} --krt_dir {data_dir}/KRT --framelist_test {framelist_test}  --result_path {result_path} --test_segment "./test_segment.json" --lambda_screen 1 --model_path {best_model_path}  --arch base  --project_name {project_name} --author_name {wandb_author_name} --val_batch_size {val_batch_size}  --num_freq_comp_outsourced {num_freq_comp_outsourced} --camera_configs_path {camera_configs_path}')