...
for num_freq_comp_outsourced in num_freq_comp_outsourced_list:
    ...
    class TracedDecoder(torch.nn.Module):
        ...
        def forward(self, v, z_outsource):
            view_code = self.model_dec.relu(self.model_dec.view_fc(v))
            z_code_outsource = self.model_dec.relu(self.model_dec.z_fc_outsource(z_outsource))
            feat_outsource = torch.cat((view_code, z_code_outsource), 1)
            texture_code_outsource = self.model_dec.relu(self.model_dec.texture_fc_outsource(feat_outsource))
            texture_outsource = self.model_dec.texture_decoder_outsource(texture_code_outsource)
    ...
    sample_z_outsource = torch.zeros(batch_size, 256).to(device)

    # Trace the model
    with torch.no_grad():
        traced_model = torch.jit.trace(traced_decoder, (sample_v, sample_z_outsource))

    # Warm up the traced model
    with torch.no_grad():
        for _ in range(10):
            _ = traced_model(sample_v, sample_z_outsource)
    ...
    for i in tqdm(range(total_inference)):
        ...
        with torch.no_grad():
            texture_outsource = traced_model(v, z_outsource)
