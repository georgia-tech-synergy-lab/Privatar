...
framelist_empirical_attack = f"/home/jianming/work/Privatar_prj/experiment_scripts/empirical_attack/selected_expression_frame_list.txt"
...
for i, data in enumerate(test_loader):
    refer_photo = data["photo"].to(device)
...
for j, data_test in enumerate(test_loader):
    for i in tqdm(range(len(collect_overall_refer_components))):
        for freq_base_id in range(1,16):
            ...
            vert_ids = data_test["vert_ids"].to(device)
            uvs = data_test["uvs"].to(device)
            uv_ids = data_test["uv_ids"].to(device)
            avg_tex = data_test["avg_tex"].to(device)
            verts = data_test["aligned_verts"].to(device)
            mask = data_test["mask"].to(device)
            batch, channel, height, width = avg_tex.shape
            photo = data_test["photo"].to(device)
