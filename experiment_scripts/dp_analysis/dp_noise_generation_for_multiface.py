...
#########################
### Please modify!!
folder_path = f'{path_to_privatar}/testing_results/multiface/latent_code'
#########################
file_dump_name_suffix = "multiface_decoder"
mutual_info_bound_list = [4, 3, 1, 0.1, 0.01]
posterious_successful_rate_list = [0.98, 0.827, 0.4, 0.09, 0.035] # corresponding to `mutual_info_bound_list = [1, 0.1]`
...
def dp_noise_calculation(l2_normd_data, prior_successful_rate, posterious_successful_rate_list):
  """
  ...
  c_square = l2_normd_data * l2_normd_data
...
np.save(f"./l2norm/l2norm_z_completed_offloaded.npy", l2_normd)
...
for i, sigma in enumerate(sigma_under_various_psr):
  ...
  np.save(f"./generated_dp_noise/dp_noise_completed_offloaded_{file_dump_name_suffix}_{mutual_info_bound_list[i]}.npy", dp_noise)
