...
for j, outsource_freq_num in enumerate(outsource_freq_list):
    ...
    if outsource_freq_num<16:
        ...
        for i, v in enumerate(mutual_info_bound_list):
            ...
            np.save(f"./noise_covariance/pac_noise{file_dump_name_suffix}_{outsource_freq_num}_{v}.npy", noise_s)
            print(f"when num_freq_comp_outsourced={outsource_freq_num}, Accumulation of simga_i (trace) of noise covariance = {np.sum(noise_s)} when MI = {v}")
    ...
    if outsource_freq_num > 0:
        ...
        for i, v in enumerate(mutual_info_bound_list):
            noise_s_outsource = np.empty_like(s_outsource)
            for dim_id in range(noise_s_outsource.shape[0]):
                noise_s_outsource[dim_id] = s_outsource[dim_id] * acc_s_outsource / 2 / v
            np.save(f"./noise_covariance/pac_noise_outsource{file_dump_name_suffix}_{outsource_freq_num}_{v}.npy", noise_s_outsource)
            print(f"when num_freq_comp_outsourced={outsource_freq_num}, Accumulation of simga_i (trace) of outsourced noise covariance = {np.sum(noise_s_outsource)} when MI = {v}")
